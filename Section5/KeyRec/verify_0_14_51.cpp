...
void impose ( u64 & key0, u64 & key1, u64 & N0, u64 & N1 )
{
    ...
    for ( int i = 0; i < 64; i++ )
     ...
     //if ( id[1] )
     x[100] = id[4] ^ x[33]*k[33] ^ x[33]*k[97] ^ x[33] ^ x[36]*k[36] ^ x[36]*k[100] ^ x[36] ^ x[58]*k[58] ^ x[58]*k[122] ^ x[97] ^ x[122] ^ k[33]*k[97] ^ k[33] ^ k[36]*k[100] ^ k[36] ^ k[58]*k[122] ^ k[97] ^ k[100] ^ k[122] ^ 1;

     //if ( id[2] )
     x[95] = id[5] ^ x[28]*k[28] ^ x[28]*k[92] ^ x[28] ^ x[31]*k[31] ^ x[31]*k[95] ^ x[31] ^ x[53]*k[53] ^ x[53]*k[117] ^ x[53] ^ x[92] ^ x[117] ^ k[28]*k[92] ^ k[28] ^ k[31]*k[95] ^ k[31] ^ k[53]*k[117] ^ k[53] ^ k[92] ^ k[95] ^ k[117];
     ...
     */
     ...
     //if ( id[6] )
     //x[4] = id[5] ^ x[7] ^ x[37] ^ x[61] ^ x[68] ^ x[71]*k[7] ^ x[71] ^ x[101]*k[37] ^ x[101] ^ x[125] ^ k[4] ^ k[7] ^ k[37] ^ k[61] ^ k[68] ^ k[125] ^ 1;

     //if ( id[7] )
     //x[1] = id[6] ^ x[25] ^ x[32] ^ x[35] ^ x[65]*k[1] ^ x[65] ^ x[89] ^ x[96] ^ x[99]*k[35] ^ x[99] ^ x[106]*k[42] ^ k[1] ^ k[25] ^ k[32] ^ k[35] ^ k[89] ^ k[96] ^ k[106] ^ 1;

     //if ( id[8] )
     x[47] = id[6] ^ x[23] ^ x[54] ^ x[57] ^ x[87]*k[23] ^ x[87] ^ x[111] ^ x[118] ^ x[121]*k[57] ^ x[121] ^ k[23] ^ k[47] ^ k[54] ^ k[57] ^ k[111] ^ k[118];

     //if ( id[9] )
     //x[16] = x[23] ^ x[26] ^ x[56] ^ x[80] ^ x[87] ^ x[90]*k[26] ^ x[90] ^ x[97]*k[33] ^ x[120]*k[56] ^ x[120] ^ k[16] ^ k[23] ^ k[26] ^ k[56] ^ k[80] ^ k[87] ^ k[97] ^ 1;

     //if ( id[10] )
     //x[86] = id[6] ^ x[19]*k[19] ^ x[19]*k[83] ^ x[19] ^ x[22]*k[22] ^ x[22]*k[86] ^ x[22] ^ x[44]*k[44] ^ x[44]*k[108] ^ x[44] ^ x[83] ^ x[108] ^ k[19]*k[83] ^ k[19] ^ k[22]*k[86] ^ k[22] ^ k[44]*k[108] ^ k[44] ^ k[83] ^ k[86] ^ k[108];
