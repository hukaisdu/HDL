...
void impose ( u64 & key0, u64 & key1, u64 & N0, u64 & N1 )
{
    ...
    //if ( id[4] )
    x[42] = id[5] ^ x[2] ^ x[12] ^ x[66] ^ x[76]*k[12] ^ x[76] ^ x[83]*k[19] ^ x[106]*k[42] ^ x[106] ^ k[2] ^ k[9] ^ k[12] ^ k[42] ^ k[66] ^ k[73] ^ k[83];
    ...
    //if ( id[6] )
    //x[4] = 1 ^ x[11] ^ x[14] ^ x[21] ^ x[44] ^ x[68] ^ x[75] ^ x[78]*k[14] ^ x[78] ^ x[85]*k[21] ^ x[85] ^ x[108]*k[44] ^ x[108] ^ k[4] ^ k[11] ^ k[14] ^ k[21] ^ k[44] ^ k[68] ^ k[75] ^ k[85];

    //if ( id[7] )
    //x[16] = x[40] ^ x[47] ^ x[50] ^ x[80]*k[16] ^ x[80] ^ x[104] ^ x[111] ^ x[114]*k[50] ^ x[114] ^ k[16] ^ k[40] ^ k[47] ^ k[50] ^ k[104] ^ k[111] ^ 1;

    //if ( id[8] )
    x[124] = id[6] ^ x[18]*k[18] ^ x[18]*k[82] ^ x[18] ^ x[60]*k[60] ^ x[60]*k[124] ^ x[60] ^ x[82] ^ k[18]*k[82] ^ k[18] ^ k[60]*k[124] ^ k[60] ^ k[82] ^ k[124];
